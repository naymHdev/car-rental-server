...
const findResources = async<T>(Model: Model<T>, QueryId: Types.ObjectId) => {
    if (!QueryId) {
        throw new AppError(httpStatus.NOT_FOUND, `Id required to find ${Model.modelName} related data`)
    }
    const resource = await Model.findById(QueryId)
    if (!resource) {
        throw new AppError(httpStatus.NOT_FOUND, `${QueryId.toString()} in ${Model.modelName} not found`)
    }
    return { [Model.modelName.toLowerCase()]: resource }
}

const findAllResources = async<T>(Model: Model<T>, Query: Record<string, unknown>, searchField: string[]) => {
...
const deleteResources = async <T, k extends keyof T>(Model: Model<T>, deleteId: Types.ObjectId, ownerId: Types.ObjectId, owner: k) => {
    ...
    if (owner in resource && resource[owner] instanceof Types.ObjectId) {
        ...
        throw new AppError(httpStatus.NOT_FOUND, `${String(owner)} field not found in ${Model.modelName}`);
    }
    ...
    return { message: `Id:${deleteId} of ${Model.modelName} has been deleted successfully` }
}
